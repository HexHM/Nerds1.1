...
def select_grid_or_color(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        if freeze_frame is not None:
            ...
            freeze_frame = None
        elif grid_edit_enabled and len(grid_points) < 2:
            grid_points.append((x, y))
            print(f"Grid point selected: {x}, {y}")
        if len(grid_points) == 2:
            generate_grid()
            print("Grid fully defined.")

def toggle_grid_edit():
    global grid_edit_enabled
    grid_edit_enabled = not grid_edit_enabled
    print(f"Grid edit {'enabled' if grid_edit_enabled else 'disabled'}")
