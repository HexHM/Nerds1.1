...
def detect_grid_lines(frame):
    """
    Detects grid lines from the ground (if painted) using edge detection and Hough Line Transform.
    ...
    # Step 4: Use Hough Line Transform to detect lines in the edge-detected image
...
def draw_results(frame, worker_contours, lines, machine_area, machine_enabled):
    # Draw grid lines detected from the ground
...
try:
    ...
    while True:
        ...
        # Detect the grid lines from the frame
        lines = detect_grid_lines(frame)
